...
#define CY_FX_EP_BULK_VIDEO_PKTS_COUNT  (0x3)//(0x10)          /* 16 packets (burst of 16) per DMA buffer. */
...
#define CY_FX_UVC_BUF_FULL_SIZE        (3046)//(CY_FX_UVC_STREAM_BUF_SIZE - 16)

/* Number of DMA buffers per GPIF DMA thread. */
#define CY_FX_UVC_STREAM_BUF_COUNT     (8)//(4)  // change to 7 from 4 for low res
